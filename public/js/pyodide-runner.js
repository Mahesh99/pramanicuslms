...
`);
  ...
  /**
   * Markdown-rendered content (ReactMarkdown) produces plain
   * <pre><code class="language-python">...</code></pre> fences, without the
   * .code-block/.cb-header wrapper the legacy notes used. Wrap those in the
   * same structure so the existing Run-button logic above can be reused.
   */
  function enhanceMarkdownCodeBlocks() {
    document.querySelectorAll('.content-md pre > code, .content .md-code-block pre > code').forEach((codeEl) => {
      const pre = codeEl.parentElement;
      if (!pre || pre.dataset.enhanced === '1') return;
      const className = codeEl.className || '';
      const langMatch = className.match(/language-(\w+)/);
      if (!langMatch || !/^py(thon)?$/.test(langMatch[1])) return;

      pre.dataset.enhanced = '1';

      const block = document.createElement('div');
      block.className = 'code-block md-code-block';
  ...
  function initPlayground() {
    ...
    if (!playground || playground.dataset.enhanced === '1') return;
    playground.dataset.enhanced = '1';
  ...
  // Used by Next.js client after injecting module HTML / mounting playground
  globalThis.__pramanicusEnhanceCodeBlocks = function () {
    enhanceMarkdownCodeBlocks();
    initCodeBlocks();
    initPlayground();
    initRuntimeBanner();
